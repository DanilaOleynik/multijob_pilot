...
def killOrphans():
    ...
    for line in processes.split('\n'):
        ...
        if ids:
            ...
            elif 'pilots_starter_titan' in args:
                pUtil.tolog("Ignoring Pilot Launcher: pid=%s, ppid=%s, args='%s'" % (pid, ppid, args))
            elif 'pilot.py' in args:
                pUtil.tolog("Ignoring other pilot: pid=%s, ppid=%s, args='%s'" % (pid, ppid, args))
            #elif 'pilots_starter_titan_2.py' in args:
            #    pUtil.tolog("Ignoring Pilot Launcher: pid=%s, ppid=%s, args='%s'" % (pid, ppid, args))
            #elif 'pilots_starter_titan_3.py' in args:
            #    pUtil.tolog("Ignoring Pilot Launcher: pid=%s, ppid=%s, args='%s'" % (pid, ppid, args))
            #elif 'pilots_starter_titan_4.py' in args:
            #    pUtil.tolog("Ignoring Pilot Launcher: pid=%s, ppid=%s, args='%s'" % (pid, ppid, args))
            elif ppid == '1':
                count += 1
...
def getMaxMemoryUsageFromCGroupsOld():
    """ Read the max_memory from CGROUPS file memory.max_usage_in_bytes"""
