...
#                                    continue
                                ...
                                if os.path.exists(logfile):
                                    ...
                                    logfileAlreadyCopied = pUtil.isLogfileCopied(_site.workdir, _job.jobId)
    ...
    if rc:
        ...
        try:
            ...
            if status and type == "log":
                ...
                pUtil.createLockFile(env['jobrec'], job.workdir, lockfile="LOGFILEREGISTERED")
    elif rc == False:
...
def getDispatcherDictionary(_diskSpace, tofile, num_of_job):
...
def get_backfill(partition="titan", max_nodes = None):
    ...
    res = {}
    pUtil.tolog ("Available resources in %s partition" % partition)
    pUtil.tolog (showbf_str)
    if showbf_str:
        shobf_out = showbf_str.splitlines()
        pUtil.tolog ("Fitted resources")
        for l in shobf_out[2:]:
            ...
            pUtil.tolog ("Nodes: %s, Walltime (str): %s, Walltime (min) %s" % (nodes, d[3], wal_time_min))
            res.update({nodes: wal_time_min}) 
    
    return res

def get_hpc_resources(partition="titan", max_nodes = None, min_nodes = 1, min_walltime = 30):

    pUtil.tolog("Looking for gap more than '%s' min" % min_walltime)
    nodes = min_nodes
    walltime =  min_walltime      
    default = True
    
    backfill = get_backfill(partition="titan", max_nodes=max_nodes)
    if backfill:
...
def getNewJob(tofile=True, num_of_job = 350):
    """ Get a new job definition from the jobdispatcher """
    ...
    num_of_job = nodes
    pUtil.tolog("Pilot will request: %s jobs" % num_of_job)
    ...
    jNode, prodSourceLabel, pilotErrorDiag = getDispatcherDictionary(_diskSpace, tofile, num_of_job)
    if jNode == {}:
    ...
    if not env['jobRequestFlag']:
        ...
        pUtil.tolog("Collecting of %s jobs took: %s sec." % (len(jobs), getJobs_time))
        ...
        for job in jobs:
            ...
            if job.has_key('logGUID'):
                logGUID = job['logGUID']
                if logGUID == "NULL" or logGUID == "":
                    job['logGUID'] = pUtil.getGUID()
                    pUtil.tolog("!!WARNING!!1200!! Server returned NULL logGUID. Using generated logGUID: %s" % (job['logGUID']), tofile=tofile)
                     
            else:
                job['logGUID'] = pUtil.getGUID()
                pUtil.tolog("!!WARNING!!1200!! Server returned NULL logGUID. Using generated logGUID: %s" % (job['logGUID']), tofile=tofile)
     
        #jobs.append(data) # array of dictionary
        ...
        #except Exception,e:
        #    env['nSent'] = 0
        #    pUtil.tolog("Default nSent: %d" % (env['nSent']))
        #else:
    ...
    '''  Looks like, that this part should go to setup procedures 
    ...
    ''' Should be aplied before launching of job 
    ...
    try: 
        jf = open("Jobs.json", "w")
        json.dump(jobs, jf, ensure_ascii = True, encoding='ascii')
        jf.close()
    except Exception,e:
    ...
    for j in jobs: 
        newJob = Job.Job()
        newJob.setJobDef(j)  # fill up the fields with correct values now
        newJob.datadir = env['thisSite'].workdir + "/PandaJobsData" 
...
def getJob():
    ...
    trial = 1
    t0 = time.time()
    pUtil.tolog("Pilot will attempt jobs download for a maximum of %d seconds" % (env['getjobmaxtime']))
    while int(time.time() - t0) < env['getjobmaxtime']:
        lim_of_job = 1
        if 'HPC_Titan' in pUtil.readpar("catchall"):
            lim_of_job = 350
        job, pilotErrorDiag = getNewJob(num_of_job=lim_of_job)
        if not job:
            ...
            env['number_of_jobs'] = len(job)
            pUtil.tolog("Increased job counter to %d" % (env['number_of_jobs']))
            
            #os.environ["PanDA_TaskID"] = job.taskID  is really needed anywhare? 
            #pUtil.tolog("Task ID set to: %s" % (job[0].taskID))
            break

    if not job:
        errorText = "!!FINISHED!!0!!No job received from jobDispatcher"
        ec = -1 # Nothing to execute: cleanup and exit
        '''
        ...
        if "No availible resources" in pilotErrorDiag:
            errorText = "!!FINISHED!!0!!Have no availble resources"
        else:
    ...
    try:
        ...
        while True:
            ...
            if ec != 0:
                ...
                '''
                for j in env['job']:
                    pUtil.tolog("Using job definition id: %s" % (j.jobDefinitionID))            
                ''' 
            # verify any contradicting job definition parameters here
            '''
            try:
                ec, pilotErrorDiag = thisExperiment.postGetJobActions(env['job'][0]) # take definition of first job for setup
                if ec == 0:
            ...
            if env['isServerUpdated']:
                ...
                if bPID:
                    ...
                    for j in env['jobDic']['prod'][1]:
                        ...
                        if globalSite:
                            pUtil.postJobTask(j, globalSite, globalWorkNode, env['experiment'], jr=False)
                            env['logTransferred'] = True
                    pUtil.tolog("Killing process: %d" % (env['jobDic'][k][0]))
                    killProcesses(env['jobDic']['prod'][0], env['jobDic']['prod'][2])
                    # move this job from env['jobDic'] to zombieJobList for later collection
                    env['zombieJobList'].append(env['jobDic']['prod'][0]) # only needs pid of this job for cleanup
                    del env['jobDic']['prod']
