...
class PandaServerClient:
    ...
    def getNodeStructure(self, job, site, workerNode, spaceReport=False, log=None):
        ...
        if job.hpcStatus == "Rejected":
            node['jobSubStatus'] = 'pilot_closed'
        else:
            node['jobSubStatus'] = ''
        ...
        if job.startTime:
            node['startTime'] = job.startTime

        if job.endTime:
            node['endTime'] = job.endTime
    ...
    def getPayloadMetadataFilename(self, workdir, jobId, altloc=""):
        ...
        _filename = os.path.join(workdir, jobId + "_jobReport.json")
