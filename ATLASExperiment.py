...
#            cls.__instance = super(ATLASExperiment, cls).__new__(cls)
    ...
    def getJobExecutionCommand(self, job, jobSite, pilot_initdir):
        ...
        asetup_options = " "
        tolog("asetup path: %s" % asetup_path)
        # Local software path
        ...
        if self.__atlasEnv:
            ...
            tolog("Job release: %s" % job.release)
            tolog("Job nomepackage: %s" % job.homePackage)
            ...
            cmd['interpreter'] = ''
            cmd['environment'] = asetup_path + asetup_options
            trf = job.trf
            ...
            if analysisJob:
                ...
                cmd['payload'] = "%s %s" % (trf, job.jobPars)
                cmd['parameters'] = ''
