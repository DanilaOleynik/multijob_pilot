...
#                                                                               self.__env['job'].homePackage)
            ...
            if pid_1: # parent process
                ...
                if (pUtil.readpar('catchall') and "HPC_Titan" in pUtil.readpar('catchall')):
                    self.__env['update_freq_server'] = 15 * 60
                    pUtil.tolog("PanDA Server update ferquency: %s s." % self.__env['update_freq_server'])
            ...
            try:
                for j in self.__env['job']:
                    state = getFileState(j.logFile, self.__env['thisSite'].workdir, j.jobId, ftype="output")
